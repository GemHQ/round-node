...
(function() {
  var Account, Application, AssetTypes, Client, Promise, SCHEMES, User, Users, Wallet, promisify, set_application;
  ...
  module.exports = Client = (function() {
    ...
    Client.prototype.assetTypes = function(_arg) {
      var client, network, networksResource;
      network = (_arg != null ? _arg : {
        network: 'bcy'
      }).network;
      client = this;
      networksResource = this.resources.networks();
      networksResource.list = promisify(networksResource.list);
      return networksResource.list().then(function(data) {
        var assetTypes, assetTypesResource, bcyNetwork;
        bcyNetwork = data.elements.reduce(function(acc, next) {
          if (next.name === network) {
            return next;
          }
        });
        ...
        assetTypes = new AssetTypes({
          client: client,
          resource: assetTypesResource
        });
        return assetTypes.loadCollection();
      }).then(function(assetTypesResource) {
        return assetTypesResource.list();
    ...
    Client.prototype.user = function(_arg) {
      var email, fetch, key, resource, url;
      email = _arg.email, key = _arg.key, url = _arg.url, fetch = _arg.fetch;
      if (key) {
        if (!url) {
          return Promise.reject(new Error('must provide a url'));
        }
        resource = this.resources.user_query("" + url + "/users/" + key);
        resource.get = promisify(resource.get);
        return resource.get().then((function(_this) {
          return function(resource) {
            return new User({
              resource: resource,
              client: _this
    ...
    Client.prototype.confirm_email = function(_arg) {
      var email_confirmation_token, key, resource, url;
      email_confirmation_token = _arg.email_confirmation_token, key = _arg.key, url = _arg.url;
      resource = this.resources.user_query("" + url + "/users/" + key);
      resource.confirm_email = promisify(resource.confirm_email);
      return resource.confirm_email({
    ...
    Client.prototype.wrapUserResource = function(_arg) {
      var resource;
